...
export class LocomotionPolicy extends PolicyModule {
  private locomotion: LocomotionHelper | null;
  ...
  async init(context: PolicyRunnerContext): Promise<void> {
    ...
    const locomotionConfig: LocomotionConfig = {
      ...(this.config.tracking as LocomotionConfig | undefined),
      policy_joint_names: policyJointNames,
    };
    this.locomotion = new LocomotionHelper(locomotionConfig);
  }

  reset(state?: PolicyState): void {
    this.locomotion?.reset(state);
  }
  ...
  getContext(): Record<string, unknown> {
    return {
      tracking: this.locomotion, // Use 'tracking' key for compatibility with observations
    };
  }
}
